...
public class ProxiedUserDetails implements UserDetails {
     ...
     * {@link DatawaveUser#getUserType()} is {@link DatawaveUser.UserType#USER}. If no proxied principal is a user principal, then the first principal in the
     * list is returned.
     *
    ...
    public DatawaveUser getPrimaryUser() {
        ...
        return proxiedUsers.stream().filter(u -> u.getUserType() == DatawaveUser.UserType.USER).findFirst().orElse(first);
    }
    ...
    public Set<String> getProxyServers() {
        ...
        Set<String> proxyServers = proxiedUsers.stream()
                .filter(u -> u.getUserType() == DatawaveUser.UserType.SERVER)
